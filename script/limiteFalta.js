...
function mostrarResultado(porcentagemFaltas, faltas, limiteFaltas) {
    const mensagemLimiteFaltasHTML =
        ...
        "<input class='_range slider pontoFaltas no-thumb' name='limiteFaltas' type='range' min='0' max='" + limiteFaltas + "' value='" + faltas + "' step='1' style='background-image: -webkit-gradient(linear, 0% 0%, 100% 0%, color-stop(" + faltas / limiteFaltas + ", #395B64), color-stop(" + faltas / limiteFaltas + ", #667074)); cursor: initial !important;' disabled>" +
        "</div>" +
...
document.querySelectorAll("[data-cargaHoraria-input]").forEach(function (el) {
    el.oninput = function () {
         ...
         * @type {string}
         */
        var style = 'background-image: -webkit-gradient(linear, 0% 0%, 100% 0%, color-stop(' + valPercent + ', #395B64), color-stop(' + valPercent + ', #667074));';
        el.style = style;
    };
    el.oninput();
});
